...
const [showAdminBroadcast, setShowAdminBroadcast] = useState(false);
  ...
  const handleSaveClick = () => {
    if (editingField === 'email' && !isEmailValid(formValues.newValue)) { 
        setModal({ show: true, type: 'error', title: 'E-mail Inválido', message: 'El formato ingresado no es correcto. Ejemplo: usuario@correo.com' }); 
        return; 
    }
    ...
    setModal({ show: true, type: 'confirm', title: 'Confirmar', message: `¿Modificar ${editingField === 'username' ? 'usuario' : editingField === 'email' ? 'e-mail' : editingField}?`, onConfirm: processUpdate });
  };
  ...
  const handlePublicarUpdate = async () => {
    ...
    try {
        ...
        setModal({ 
            show: true, type: 'success', title: '¡Difusión Completada!', 
            message: 'La actualización se ha guardado y enviado por correo a todos los hermanos de la congregación.' 
        });
        setFormValues({ ...formValues, adminNote: '', descripcionLarga: '' });
    } catch (err) {
        setModal({ show: true, type: 'error', title: 'Error de Envío', message: 'No se pudo procesar la difusión masiva.' });
    } finally { setLoading(false); }
  };

  const triggerConfirmPublicar = () => {
    if (!formValues.adminNote || !formValues.descripcionLarga) {
        setModal({ show: true, type: 'error', title: 'Campos Incompletos', message: 'Debe escribir un título y una descripción detallada antes de publicar.' });
        return;
    }
    setModal({ 
        show: true, type: 'confirm', title: '¿Confirmar Difusión?', 
        message: 'Esta acción enviará un correo electrónico a TODOS los usuarios activos. ¿Desea continuar?', 
    ...
    return (
        <div className="mt-4 p-6 rounded-2xl border bg-jw-body border-jw-border animate-in slide-in-from-top-4">
            ...
            <div className="space-y-4 max-w-md text-left">
                <h3 className="text-sm font-bold text-jw-navy mb-6 uppercase italic tracking-tight">Modificar {editingField === 'contacto' ? 'Teléfono' : editingField === 'email' ? 'Correo Electrónico' : editingField}</h3>
                {editingField === 'password' ? (
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] uppercase font-bold text-gray-400 italic">{editingField === 'email' ? 'Nueva dirección' : 'Nuevo dato'}</label>
                        <input type={editingField === 'email' ? 'email' : 'text'} placeholder={editingField === 'email' ? 'ejemplo@correo.com' : 'Escriba aquí'} className="w-full p-3 rounded-xl border border-jw-border bg-white outline-none focus:ring-2 focus:ring-jw-blue text-jw-navy" value={formValues.newValue} onChange={(e) => setFormValues({...formValues, newValue: (editingField === 'contacto' ? e.target.value.replace(/\D/g, '') : e.target.value)})} />
                    </div>
                )}
    ...
    <div className="p-8">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-jw-body p-6 rounded-2xl border border-jw-border">
            <div className="text-left flex-1">
                ...
                <p className="text-[10px] text-gray-400 mt-1 uppercase tracking-widest font-bold italic">
        ...
        <p className="text-[11px] text-gray-400 italic font-light mt-6 text-left">
        ...
        {user?.es_admin_local && showAdminBroadcast && (
            <div className="mt-8 p-8 bg-amber-50 border-2 border-dashed border-amber-200 rounded-[2rem] animate-in slide-in-from-top-4 duration-500 text-left">
                <div className="flex items-center gap-3 mb-6">
                    <div className="p-3 bg-amber-100 rounded-2xl text-amber-600 shadow-inner"><ShieldAlert size={24}/></div>
                    <div>
                        <h3 className="text-amber-900 font-black text-sm uppercase tracking-tighter">Difusión de Seguridad</h3>
                        <p className="text-[9px] text-amber-600 font-bold uppercase italic">Panel de actualización y envío masivo</p>
                    </div>
                </div>

                <div className="space-y-6">
                    <div>
                        <label className="text-[10px] font-black text-amber-700 uppercase ml-2 mb-1 block italic">Título Breve</label>
                        <input 
                            type="text"
                            className="w-full p-4 rounded-2xl border border-amber-200 text-sm outline-none focus:ring-2 focus:ring-amber-400 bg-white font-bold text-jw-navy"
                            placeholder="Ej: Nuevos correos sospechosos"
                            value={formValues.adminNote}
                            onChange={(e) => setFormValues({...formValues, adminNote: e.target.value})}
                        />
                    </div>

                    <div>
                        <label className="text-[10px] font-black text-amber-700 uppercase ml-2 mb-1 block italic">Descripción para el Correo (Soporta HTML)</label>
                        <textarea 
                            className="w-full p-6 rounded-3xl border border-amber-200 text-sm outline-none focus:ring-2 focus:ring-amber-400 bg-white font-light min-h-[250px] text-jw-navy"
                            placeholder="Redacte aquí el mensaje detallado..."
                            value={formValues.descripcionLarga}
                            onChange={(e) => setFormValues({...formValues, descripcionLarga: e.target.value})}
                        />
                    </div>
          ...
          <div className="p-8 flex flex-col md:flex-row gap-10 items-center md:items-start text-left">
            ...
            <div className="flex-1 space-y-6 pt-4 w-full text-jw-navy">
              ...
              <div>
                ...
                <div className="space-y-3 mt-4 border-l-4 border-jw-blue/10 pl-5 text-jw-navy">
                    ...
                    <AddressLine icon={<Landmark />} value={user?.provincia} label="Provincia / Estado" />
...
function AddressLine({ icon, value, label }) {
    ...
    return (
        <div className="flex items-center gap-3 text-slate-800 py-0.5"> 
