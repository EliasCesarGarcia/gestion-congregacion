...
import { 
  ...
  FileText, Trash2, ArrowRight, Lock 
} from 'lucide-react';
...
function ProfilePage() {
  ...
  setLoading(true);
  try {
    const response = await axios.post('/api/request-pin', { 
      email: user.email, 
    ...
    if (response.status === 200) {
      ...
      setVerificationStep(2); // Pasamos a la caja de texto del PIN
    }
  } catch (err) {
    console.error("Error al pedir PIN:", err.response?.data || err.message);
    setModal({ 
      show: true, 
      type: 'error', 
      title: 'Error de Envío', 
      message: 'El servidor no pudo generar el código. Revisa la terminal del backend.' 
    });
  } finally {
    setLoading(false);
  }
  ...
  const handleSaveClick = () => {
    if (editingField === 'email' && !isEmailValid(formValues.newValue)) {
      setModal({ show: true, type: 'error', title: 'Email Inválido', message: 'Formato: nombre@empresa.com' });
      return;
    }
  ...
  const handleConfirmDelete = () => {
    setModal({
      ...
      message: 'Su acceso será revocado. Deberá solicitar una nueva clave temporal al secretario para volver a entrar. ¿Desea continuar?',
      onConfirm: async () => {
        try {
  ...
  return (
    <div className="bg-jw-body min-h-screen py-10 px-4 relative font-sans text-left">
      {toast && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[200] bg-jw-blue text-white px-8 py-3 rounded-full shadow-2xl animate-bounce text-sm font-bold tracking-widest uppercase">
          ✅ CÓDIGO ENVIADO
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-jw-navy/60 backdrop-blur-md p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden border border-jw-border animate-in zoom-in duration-200">
            <div className="p-8 bg-jw-navy text-white text-center border-b-4 border-jw-blue">
              ...
              <h3 className="text-xl font-bold italic tracking-tight text-white">Verificar Identidad</h3>
            </div>
            <div className="p-8 text-center">
              {verificationStep === 1 ? (
                <>
                  <p className="text-gray-500 text-sm mb-6 font-light italic">
                    Para modificar su <b>{editingField === 'username' ? 'nombre de usuario' : editingField}</b>, enviaremos un código a:<br/>
                    <span className="text-jw-blue font-bold text-lg not-italic">{maskEmail(user.email)}</span>
                  </p>
                  <button onClick={handleSendCode} disabled={loading} className="w-full bg-jw-blue text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg hover:bg-jw-navy transition-all">
                ...
                <>
                  ...
                  <input 
                    ...
                    maxLength="6" 
                    value={pin} 
                    className="w-full text-center text-3xl tracking-[0.5em] font-black p-4 bg-jw-body rounded-2xl border-2 border-jw-blue outline-none mb-6 text-jw-navy" 
                    onChange={(e) => setPin(e.target.value.replace(/\D/g, ""))} 
                  />
                  <button onClick={handleVerifyCode} disabled={loading || pin.length < 6} className="w-full bg-green-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest text-xs shadow-lg hover:bg-green-700 transition-all">
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <section className="bg-white rounded-xl shadow-sm border border-jw-border overflow-hidden">
          <div className="p-5 bg-jw-navy text-white border-b-4 border-jw-blue">
            <h2 className="text-lg font-light italic flex items-center gap-3">
              <Shield className="w-5 h-5 text-jw-accent"/> Administración de Cuenta
            </h2>
          </div>
          <div className="p-8 space-y-8 text-sm">
            ...
            {/* FORMULARIO DE EDICIÓN (PASO 3) */}
            {editingField && verificationStep === 3 && (
              <div className="mt-4 p-6 bg-jw-body rounded-2xl border border-jw-border animate-in slide-in-from-top-4">
                <div className="flex items-center gap-2 mb-6 text-green-600 font-bold text-[10px] uppercase tracking-widest italic">
                  ...
                  {editingField === 'password' ? (
                    <div className="space-y-4">
                      ...
                      {formValues.newValue && !isPassStrong(formValues.newValue) && (
                        <p className="text-[10px] text-red-500 italic bg-red-50 p-2 rounded-lg text-center">Fuerza insuficiente: requiere 8 caracteres, Mayúscula, Número y Símbolo.</p>
                      )}
                    </div>
                  ) : (
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase font-bold text-gray-400 tracking-widest italic">Dato Actual: {user[editingField]}</label>
                      <input 
                        type={editingField === 'email' ? 'email' : 'text'} 
                        placeholder={`Nuevo ${editingField === 'username' ? 'nombre de usuario' : 'e-mail'}`} 
                        className="w-full p-3 rounded-xl border border-jw-border bg-white outline-none focus:ring-2 focus:ring-jw-blue" 
                        onChange={(e) => setFormValues({...formValues, newValue: e.target.value})} 
                      />
                    </div>
                  )}
                  <div className="flex gap-2 pt-2">
                    <button onClick={handleSaveClick} disabled={loading} className="flex-1 bg-jw-blue text-white p-3 rounded-xl font-bold text-xs uppercase tracking-widest shadow-md hover:bg-jw-navy transition-all">Confirmar</button>
        ...
        <section className="bg-white rounded-xl shadow-sm border border-jw-border p-8 text-left">
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-xs text-gray-400 italic font-light text-left">Para dar de baja su cuenta temporalmente, contacte al secretario.</p>
        ...
        <section className="bg-white rounded-xl shadow-sm border border-jw-border overflow-hidden text-left">
          <div className="p-5 bg-jw-navy text-white border-b-4 border-jw-blue">
          ...
          <div className="p-8 flex flex-col md:flex-row gap-10 items-center md:items-start">
            ...
            <div className="flex-1 space-y-6 pt-4">
              ...
              <div className="bg-jw-body p-6 rounded-2xl border border-jw-border flex items-start gap-4 shadow-sm">
                <HelpCircle className="w-6 h-6 text-jw-blue shrink-0 mt-1" />
                <div className="text-[13px] text-gray-600 leading-relaxed italic">
                  <p className="font-bold not-italic text-jw-navy mb-1 leading-tight text-sm text-left">¿Necesitas ayuda?</p>
  ...
  return (
    <div className="flex justify-between items-center border-b border-gray-50 pb-4">
      <div className="text-left flex items-center gap-4">
        <div className="p-2 bg-jw-body rounded-lg text-gray-400 italic">{icon}</div>
        <div>
  ...
  return (
    <div className="relative">
      <input 
        ...
        className="w-full p-3 rounded-xl border border-jw-border bg-white outline-none text-sm focus:ring-2 focus:ring-jw-blue" 
      />
